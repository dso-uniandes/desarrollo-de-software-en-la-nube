...
@pytest.fixture()
async def created_comment(async_client: AsyncClient, created_post, logged_in_token: str):
...
async def test_create_post(async_client: AsyncClient, registered_user: dict, logged_in_token: str):
    ...
    assert response_data["user_id"] == registered_user["id"]
...
@pytest.mark.anyio
async def test_create_missing_data(async_client: AsyncClient, logged_in_token: str):
    response = await async_client.post(
        "/post",
        json={},
        headers={"Authorization": f"Bearer {logged_in_token}"}
...
@pytest.mark.anyio
async def test_create_comment(async_client: AsyncClient, registered_user: dict, created_post: dict, logged_in_token):
    body = "Test Comment"
    response = await async_client.post(
        "/comment",
    ...
    response_data = response.json()
    assert response_data["body"] == body
    assert response_data["post_id"] == created_post["id"]
    assert response_data["user_id"] == registered_user["id"]
    assert "id" in response_data
